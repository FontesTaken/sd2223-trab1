...
public class RestFeedsClient extends RestClient implements Feeds {
	...
	private Result<Long> clt_postMessage(String user, String pwd, Message msg) {
		...
		Response r = target.path(user).queryParam(FeedsService.PWD, pwd).request().accept(MediaType.APPLICATION_JSON)
	...
	private Result<Void> clt_removeFromPersonalFeed(String user, long mid, String pwd) {
		...
		Response r = target.path(user).path(String.valueOf(mid)).queryParam(FeedsService.PWD, pwd).request().delete();
	...
	private Result<Message> clt_getMessage(String user, long mid) {
		...
		Response r = target.path(user).path(String.valueOf(mid)).request().accept(MediaType.APPLICATION_JSON).get();
	...
	private Result<List<Message>> clt_getMessages(String user, long time) {
		...
		Response r = target.path(user).path(String.valueOf(time)).request().accept(MediaType.APPLICATION_JSON).get();
	...
	private Result<Void> clt_subUser(String user, String userSub, String pwd) {
		...
		Response r = target.path("sub").path(user).path(userSub).queryParam(FeedsService.PWD, pwd).request()
	...
	private Result<Void> clt_unsubscribeUser(String user, String userSub, String pwd) {
		...
		Response r = target.path("sub").path(user).path(userSub).queryParam(FeedsService.PWD, pwd).request().delete();
	...
	private Result<List<String>> clt_listSubs(String user) {
		...
		Response r = target.path("sub").path("list").path(user).request().accept(MediaType.APPLICATION_JSON).get();
	...
	private Result<List<Message>> clt_getMessagesFromRemote(String user, String originalDomain, long time) {
		Response r = target.path(user).path(originalDomain).path(Long.toString(time)).request()
